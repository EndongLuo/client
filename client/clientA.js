...
import { NAME_MAP } from './diagnosticMap.js';
...
ws.on('open', () => {
    ...
    const payload = {
        status: encodeStatusName(message),
    };
    // console.log(payload);
...
function encodeStatusName(statusList) {
    return statusList.map(item => {
        // console.log('------', item.name);

        const name_id = NAME_MAP[item.name];

        // console.log('++++++++', name_id);


        if (!name_id) {
            throw new Error(`未知 name: ${item.name}`);
        }
